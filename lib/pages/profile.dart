...
import 'package:add_2_calendar/add_2_calendar.dart' as add_calendar;
import '../db/db.dart';
...
class Profile extends StatefulWidget {
  ...
  final Box eventsBox = Hive.box("events6");
...
class _ProfileState extends State<Profile> {
  ...
  void initState() {
    super.initState();
    defaultImage = const AssetImage('assets/default_photo.jpg');
  }
  ...
  Widget eventsMade() {
    List<Event> _events = widget.user.events;
  ...
  Widget upcomingRSVPs() {
    Iterable<Event> _events = widget.eventsBox.values.cast();
    List<Event> _available = [];

    for (Event e in _events) {
      if (e.rsvpee != null) {
        if (e.rsvpee == widget.user) {
          _available.add(e);
        }
      }
    }
    ...
    return Scaffold(
      ...
      body: (_available.isNotEmpty)
          ...
          : const Center(child: const Text("You haven't RSVPed for anything!")),
    );
  }
            ...
            Container(
              ...
              child: Row(
                children: [
                  ...
                  Column(
                    children: [
                      Text(widget.user.name, style: GoogleFonts.lexend()),
                      const Padding(padding: EdgeInsets.symmetric(vertical: 4)),
                      Text(widget.user.email, style: GoogleFonts.lexend()),
                    ],
                  ),
                ],
              ),
            ...
            Container(
                ...
                child: TextButton(
                    onPressed: () => Navigator.push(context,
                        MaterialPageRoute(builder: (context) => eventsMade())),
